...
**SUA TAREFA:**
Analise o texto a seguir. Para cada uma das 8 dimensões, forneça uma pontuação de 0 a 100. Para as 7 primeiras, avalie tanto 'coerencia' quanto 'dissonancia'. Para 'alinhamentoPAC', forneça uma única pontuação. Além disso, escreva um resumo qualitativo e conciso da sua análise, destacando os pontos mais importantes de coerência e dissonância.
...
const analysisSchema = {
    ...
    properties: {
        vector: {
            type: Type.OBJECT,
            properties: {
                alinhamentoPAC: { type: Type.INTEGER },
                proposito: { type: Type.OBJECT, properties: { coerencia: { type: Type.INTEGER }, dissonancia: { type: Type.INTEGER } } },
                mental: { type: Type.OBJECT, properties: { coerencia: { type: Type.INTEGER }, dissonancia: { type: Type.INTEGER } } },
                relacional: { type: Type.OBJECT, properties: { coerencia: { type: Type.INTEGER }, dissonancia: { type: Type.INTEGER } } },
                emocional: { type: Type.OBJECT, properties: { coerencia: { type: Type.INTEGER }, dissonancia: { type: Type.INTEGER } } },
                somatico: { type: Type.OBJECT, properties: { coerencia: { type: Type.INTEGER }, dissonancia: { type: Type.INTEGER } } },
                eticoAcao: { type: Type.OBJECT, properties: { coerencia: { type: Type.INTEGER }, dissonancia: { type: Type.INTEGER } } },
                recursos: { type: Type.OBJECT, properties: { coerencia: { type: Type.INTEGER }, dissonancia: { type: Type.INTEGER } } },
            },
        },
        summary: {
            type: Type.STRING,
