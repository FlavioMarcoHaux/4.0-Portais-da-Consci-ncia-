...
import { JournalEntry, AgentId, Session } from '../types.ts';
import { X, BookHeart, Loader2, Send, ChevronDown, Mic } from 'lucide-react';
...
const TherapeuticJournal: React.FC<TherapeuticJournalProps> = ({ onExit }) => {
    ...
    return (
        <div className="h-full w-full glass-pane rounded-2xl flex flex-col p-1 animate-fade-in">
            ...
            <main className="flex-1 overflow-y-auto p-6 no-scrollbar" data-guide-id="tool-therapeutic_journal">
                                            ...
                                            <div className="p-4 border-t border-gray-700/50 animate-fade-in" data-readable-content>
                                                ...
                                                <div className="space-y-3">
                                                    <div className="p-3 bg-gray-900/50 rounded-lg">
                                                        <h4 className="font-semibold text-gray-300">Observação:</h4>
                                                        <p className="text-sm text-gray-400 mt-1">{item.feedback.observacao}</p>
                                                    </div>
                                                     <div className="p-3 bg-gray-900/50 rounded-lg">
                                                        <h4 className="font-semibold text-gray-300">Dissonância:</h4>
                                                        <p className="text-sm text-gray-400 mt-1">{item.feedback.dissonancia}</p>
                                                    </div>
                                                    <div className="p-3 bg-indigo-900/30 border border-indigo-800/50 rounded-lg">
                                                        <h4 className="font-semibold text-indigo-300">Ação Sugerida:</h4>
                                                        <p className="text-sm text-indigo-200/90 mt-1">{item.feedback.acao}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
