...
type AnalysisState = 'idle' | 'listening' | 'processing' | 'result' | 'error';

const VerbalFrequencyAnalysis: React.FC<VerbalFrequencyAnalysisProps> = ({ onExit }) => {
    const { startSession, toolStates, setToolState, logActivity, lastAgentContext } = useStore(state => ({
    ...
    const history = toolStates.verbalFrequencyAnalysis?.history || [];
    ...
    const drawVisualizer = useCallback(() => {
        ...
        for (let i = 0; i < bufferLength; i++) {
            const barHeight = (smoothedData[i] / 255) * height * 1.2; // Increase height multiplier
    ...
    const handleStartListening = useCallback(async () => {
        ...
        try {
            ...
            const message = getFriendlyErrorMessage(err, 'Ocorreu um erro inesperado ao tentar acessar o microfone.');
            setError(message);
            setAnalysisState('error');
        }
    }, [startListening, drawVisualizer, isListening]);
    ...
    const handleStopAndAnalyze = useCallback(async () => {
        ...
        if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
            mediaRecorderRef.current.onstop = async () => {
                ...
                try {
                    ...
                    reader.onloadend = async () => {
                        ...
                        const analysisResult = await analyzeVerbalFrequency(finalTranscriptRef.current, audioData);
                        
                        const newEntry: VerbalFrequencyEntry = {
                            ...analysisResult,
                            id: `vfa-${Date.now()}`,
                            timestamp: Date.now(),
                        };
                        setToolState('verbalFrequencyAnalysis', { history: [newEntry, ...history] });
                        logActivity({
                            type: 'tool_usage',
                            agentId: agentIdForContext,
                            data: {
                                toolId: 'verbal_frequency_analysis',
            ...
            mediaRecorderRef.current.stop();
             ...
             setError("A gravação não foi iniciada corretamente.");
             setAnalysisState('error');
        }
    }, [stopListening, cleanupAudio, history, setToolState, logActivity, agentIdForContext]);
                ...
                return (
                    <div className="flex flex-col items-center justify-start h-full w-full text-center p-4 animate-fade-in">
                         <p className="text-lg text-gray-300 max-w-md mb-4">
                            Pressione o microfone para começar a falar. Desabafe sobre o que está em sua mente e coração.
                        </p>
                        <p className="text-sm text-gray-500 mb-8">
                            Ao terminar, analisaremos a frequência emocional da sua voz.
                        </p>
                        <button
                            onClick={handleStartListening}
                            className="w-24 h-24 bg-purple-600 rounded-full flex items-center justify-center hover:bg-purple-700 transition-all shadow-lg hover:scale-105"
                             ...
                             <div className="mt-12 pt-8 border-t border-gray-700/50 w-full max-w-2xl">
                                ...
                                <div className="space-y-4">
                                    {history.map(item => {
                                        ...
                                        const recommendedToolEntry = Object.entries(toolMetadata).find(
                                            ([_, meta]) => meta.title === recommendedToolName
                                        );
                                                ...
                                                {expandedId === item.id && (
                                                    <div className="p-4 border-t border-gray-700/50 animate-fade-in text-left space-y-4">
                                                        <div className="bg-gray-900/50 p-3 rounded-lg">
                                                         ...
                                                         {recommendedToolId ? (
                                                            <button
                                                                onClick={() => startSession({ type: recommendedToolId })}
                                                                className="w-full bg-purple-900/50 border border-purple-700 p-4 rounded-lg text-left hover:bg-purple-800/60 transition-colors group"
                                                            >
                                                                <h4 className="text-sm font-semibold text-purple-300 uppercase tracking-wider flex items-center gap-2">
                                                                    <Sparkles size={16}/> Ação Recomendada
                                                                </h4>
                                                                <div className="flex justify-between items-center mt-1">
                                                                    <p className="text-purple-200">
                                                                        Iniciar: <span className="font-bold">{item.acao_pac_recomendada}</span>
                                                                    </p>
                ...
                return (
                    <div className="flex flex-col items-center justify-between h-full text-center p-6 animate-fade-in">
                        <div className="flex items-center gap-2 text-red-400 animate-pulse">
                            <Mic size={24} />
                            <span className="font-semibold text-lg">Ouvindo...</span>
                        </div>
                        <canvas ref={canvasRef} width="500" height="200" className="w-full max-w-lg h-auto" />
                        <button
                            onClick={handleStopAndAnalyze}
                            className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-8 rounded-full text-lg flex items-center gap-2"
                        >
                            <Square size={20} /> Parar e Analisar
                        </button>
                    </div>
                );
            case 'processing':
                ...
                return (
                    <div className="animate-fade-in w-full max-w-2xl mx-auto space-y-6 text-center">
                        <div className="relative w-48 h-48 mx-auto flex items-center justify-center">
                            <svg className="w-full h-full" viewBox="0 0 36 36">
                                ...
                                <path
                                    ...
                                    stroke="#8b5cf6"
                                    strokeWidth="2"
                                    strokeDasharray={`${result.coerencia_score * 10}, 100`}
                                />
                            </svg>
                            <div className="absolute">
                                <span className="text-5xl font-bold text-purple-400">{result.coerencia_score}</span>
                        ...
                        {recommendedToolId ? (
                            <button
                                onClick={() => startSession({ type: recommendedToolId })}
                                className="w-full bg-purple-900/50 border border-purple-700 p-6 rounded-lg text-left hover:bg-purple-800/60 transition-colors group"
                            >
                                <h3 className="text-sm font-semibold text-purple-300 uppercase tracking-wider flex items-center gap-2">
                                    <Sparkles size={16}/> Ação de Coerência Recomendada (PAC)
                                </h3>
                                <div className="flex justify-between items-center mt-2">
                                    <p className="text-lg text-purple-200">
                                        Iniciar: <span className="font-bold">{result.acao_pac_recomendada}</span>
                                    </p>
                                    <ArrowRight className="w-6 h-6 text-purple-400 group-hover:translate-x-1 transition-transform" />
                                </div>
                            </button>
                        ) : (
                             <div className="bg-purple-900/50 border border-purple-700 p-6 rounded-lg">
                                ...
                                <p className="text-lg text-purple-200 mt-2">Para elevar sua coerência, sugerimos a ferramenta: <span className="font-bold">{result.acao_pac_recomendada}</span></p>
                            </div>
                        )}
            ...
            case 'error':
                 return (
                    <div className="flex flex-col items-center justify-center h-full text-center p-4">
                        ...
                        <p className="text-gray-300 mb-6 max-w-md">{error || speechError}</p>
        ...
        <div className="h-full w-full glass-pane rounded-2xl flex flex-col p-1 animate-fade-in">
            <header className="flex items-center justify-between p-4 border-b border-gray-700/50">
                <div className="flex items-center gap-3">
                 ...
                 <div className="flex items-center gap-4">
                    <button onClick={() => onExit(true)} className="text-gray-400 hover:text-white transition-colors" aria-label="Exit Verbal Frequency Analysis">
                        <X size={24} />
                    </button>
                </div>
            </header>
