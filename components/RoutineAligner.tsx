...
const RoutineAligner: React.FC<RoutineAlignerProps> = ({ onExit }) => {
    const { toolStates, isLoadingMessage, handleRoutineAlignerSendMessage } = useStore();
        ...
        <div className="h-full w-full glass-pane rounded-2xl flex flex-col p-1 animate-fade-in">
            <header className="flex items-center justify-between p-4 border-b border-gray-700/50">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={() => onExit(true)} className="text-gray-400 hover:text-white transition-colors" aria-label="Exit Routine Aligner">
                        <X size={24} />
                    </button>
                </div>
            </header>
            <main className="flex-1 flex flex-col overflow-hidden" data-guide-id="tool-routine_aligner">
